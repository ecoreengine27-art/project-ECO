...
import { supabase, Project, difficultyLabel, difficultyColor } from '../lib/supabase';
...
export default function ProjectsPage({ userId, onXpGained }: ProjectsPageProps) {
  ...
  async function updateStatus(project: Project, newStatus: string) {
    ...
    if (newStatus === 'completed') {
      const { data: prof } = await supabase.from('user_profiles').select('total_xp, projects_completed').eq('id', userId).maybeSingle();
      if (prof) {
        await supabase.from('user_profiles').update({
          total_xp: (prof.total_xp ?? 0) + xpGain,
          projects_completed: (prof.projects_completed ?? 0) + 1,
        }).eq('id', userId);
        onXpGained();
      }
    }
          ...
          {filtered.map(project => {
            ...
            return (
              <button
                ...
                className="card-hover flex flex-col gap-3 text-left group"
              >
