...
import { Package, Plus, Search, Trash2, ChevronDown, Recycle, Zap, X } from 'lucide-react';
import { supabase, Component, UserComponent, categoryLabel, difficultyLabel, difficultyColor } from '../lib/supabase';
...
export default function InventoryPage({ userId, onXpGained }: InventoryPageProps) {
  ...
  async function loadData() {
    const [{ data: comps }, { data: inv }] = await Promise.all([
      supabase.from('components').select('*').order('category').order('name'),
      supabase.from('user_components').select('*, component:components(*)').eq('user_id', userId),
    ]);
    setComponents(comps ?? []);
  ...
  async function addToInventory() {
    ...
    if (!error) {
      const { data: profileData } = await supabase.from('user_profiles')
        .select('total_xp, components_salvaged, co2_saved_kg')
        .eq('id', userId)
        .maybeSingle();

      if (profileData) {
        const newCo2 = (Number(profileData.co2_saved_kg) || 0) + (selectedComponent.co2_saved_g * quantity / 1000);
        await supabase.from('user_profiles').update({
          total_xp: (profileData.total_xp ?? 0) + selectedComponent.salvage_xp * quantity,
          components_salvaged: (profileData.components_salvaged ?? 0) + quantity,
          co2_saved_kg: newCo2,
        }).eq('id', userId);
      }
  ...
  async function removeFromInventory(id: string) {
    await supabase.from('user_components').delete().eq('id', id);
