...
import { Search, Filter, Clock, Zap, ChevronRight, CheckCircle, Play, BookOpen } from 'lucide-react';
import { supabase, Tutorial, TutorialProgress, difficultyLabel, difficultyColor, categoryLabel } from '../lib/supabase';
...
const DIFFICULTY_FILTERS = [0, 1, 2, 3, 4, 5];

export default function LearnPage({ userId }: LearnPageProps) {
  ...
  useEffect(() => {
    async function load() {
      const [{ data: t }, { data: p }] = await Promise.all([
        supabase.from('tutorials').select('*').order('order_index'),
        supabase.from('tutorial_progress').select('*').eq('user_id', userId),
      ]);
      setTutorials(t ?? []);
      const map = new Map<string, TutorialProgress>();
      (p ?? []).forEach(item => map.set(item.tutorial_id, item));
      setProgress(map);
      setLoading(false);
    }
    load();
  ...
  async function completeTutorial() {
    ...
    const { data: profileData } = await supabase.from('user_profiles').select('total_xp, tutorials_completed').eq('id', userId).maybeSingle();
    if (profileData) {
      await supabase.from('user_profiles').update({
        total_xp: (profileData.total_xp ?? 0) + selected.xp_reward,
        tutorials_completed: (profileData.tutorials_completed ?? 0) + 1,
      }).eq('id', userId);
    }
          ...
          {filtered.map(tutorial => {
            ...
            return (
              ...
              >
                ...
                <div className="flex-1">
                  <h3 className="text-sm font-semibold text-slate-100 leading-snug mb-1">{tutorial.title}</h3>
